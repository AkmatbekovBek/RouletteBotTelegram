...
class AdminHandler:
    ...
    def _get_all_admins(self) -> List[int]:
        ...
        with self._db_session() as db:
            try:
                ...
                self.logger.error(f"Error getting admins from DB: {e}")
                return []
    ...
    def _get_gift_cancel_keyboard(self) -> types.InlineKeyboardMarkup:
        """Клавиатура для отмены операций с подарками"""
        return types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("❌ Отмена", callback_data="admin_gift_cancel")
        )

    def _get_gift_management_keyboard(self) -> types.InlineKeyboardMarkup:
    ...
    def _get_broadcast_cancel_keyboard(self, broadcast_type: str = "") -> types.InlineKeyboardMarkup:
    ...
    async def admin_help(self, message: types.Message):
        ...
        all_admins = self._get_all_admins()
            ...
            with self._db_session() as db:
                ...
                if operation == "addcoins":
                    ...
                    if amount > current_coins:
                        await message.answer(f"❌ У пользователя только {self._format_number(current_coins)} монет")
                        return
                    new_coins = current_coins - amount
                ...
                response = (
                    ...
                    f"💰 Было: {self._format_number(current_coins)} | Стало: {self._format_number(new_coins)}\n"
                    f"📊 {operation_names[operation].title()}: {self._format_number(amount)} монет"
                )
                await message.answer(response, parse_mode="HTML")
    ...
    async def admin_gift_add_start(self, message: types.Message):
        ...
        await message.answer(
            ...
            reply_markup=self._get_gift_cancel_keyboard(),
            parse_mode="HTML"
        )
        ...
        with self._db_session() as db:
            try:
                ...
                response = (
                    ...
                    f"💰 Цена: {self._format_number(gift.price)} монет\n"
                    f"💝 Комплимент: {gift.compliment}"
                )

                await message.answer(
                    response,
                    reply_markup=self._get_gift_management_keyboard(),
                    parse_mode="HTML"
                )
    ...
    async def admin_gift_list(self, message: types.Message):
        ...
        with self._db_session() as db:
            ...
            for i, gift in enumerate(gifts, 1):
                ...
                gifts_text += f"   📎 {gift.sticker} | 💰 {self._format_number(gift.price)} монет\n"
        ...
        try:
            ...
            with self._db_session() as db:
                ...
                info_text = (
                    ...
                    f"💰 Баланс: {self._format_number(user.coins)} монет\n"
                    f"🎯 Выиграно: {self._format_number(user.win_coins or 0)} монет\n"
                    f"💸 Проиграно: {self._format_number(user.defeat_coins or 0)} монет\n"
                    f"📈 Макс. выигрыш: {self._format_number(user.max_win_coins or 0)} монет\n"
    ...
    async def bot_stats(self, message: types.Message):
        ...
        with self._db_session() as db:
            ...
            stats_text += f"👥 Всего пользователей: {self._format_number(total_users)}\n"
            stats_text += f"💰 Всего монет в системе: {self._format_number(total_coins)}\n"
            ...
            if rich_users:
                ...
                for i, user in enumerate(rich_users, 1):
                    ...
                    stats_text += f"{i}. {display_name} - {self._format_number(user.coins)} монет{admin_status}\n"
            else:
        ...
        try:
            with self._db_session() as db:
                ...
                status_msg = await message.answer(
                    f"📢 Начинаю рассылку для {self._format_number(total)} {recipient_type}...\n"
                    f"⏳ Обработано: 0/{self._format_number(total)}\n"
                    f"✅ Успешно: 0\n"
                    f"❌ Ошибок: 0",
                    reply_markup=self._get_broadcast_cancel_keyboard(broadcast_type)
                )
                    ...
                    if i % BROADCAST_BATCH_SIZE == 0 or i == total:
                        try:
                            await status_msg.edit_text(
                                f"📢 Рассылка для {self._format_number(total)} {recipient_type}...\n"
                                f"⏳ Обработано: {self._format_number(i)}/{self._format_number(total)}\n"
                                f"✅ Успешно: {self._format_number(success_count)}\n"
                                f"❌ Ошибок: {self._format_number(failed_count)}",
                                reply_markup=self._get_broadcast_cancel_keyboard(broadcast_type)
                            )
                        except Exception as e:
                ...
                if self.broadcast_cancelled:
                    result_text = (
                        f"❌ Рассылка отменена!\n\n"
                        f"📊 Итоги:\n"
                        f"👥 Всего {recipient_type}: {self._format_number(total)}\n"
                        f"⏳ Обработано: {self._format_number(i)}\n"
                        f"✅ Успешно: {self._format_number(success_count)}\n"
                        f"❌ Ошибок: {self._format_number(failed_count)}"
                    )
                else:
                    ...
                    result_text = (
                        ...
                        f"👥 Всего {recipient_type}: {self._format_number(total)}\n"
                        f"✅ Успешно: {self._format_number(success_count)}\n"
                        f"❌ Не удалось: {self._format_number(failed_count)}\n"
    ...
    async def find_user(self, message: types.Message):
        ...
        with self._db_session() as db:
            ...
            for user in found_users[:10]:
                ...
                coins = self._format_number(user.coins)
                admin_status = " 👑" if user.is_admin else ""
        ...
        try:
            ...
            with self._db_session() as db:
                ...
                response = (
                    ...
                    f"👤 Пользователь: {user.first_name or 'Без имени'}\n"
                    f"📱 Username: @{user.username or 'нет'}\n"
                    f"💰 Текущий баланс: {self._format_number(user.coins)} монет\n\n"
    ...
    async def handle_broadcast_cancel(self, callback: types.CallbackQuery):
        ...
        if not await self._check_admin(callback.message):
...
def register_admin_handlers(dp: Dispatcher):
    ...
    print("✅ Админ обработчики зарегистрированы (полная оптимизированная версия)")